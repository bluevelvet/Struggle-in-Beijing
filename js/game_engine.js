...
Game.say = function () {
	return {
		...
		deal: function (dealInfo) {
			...
			say = "<h1>" + say + "</h1>";
			document.getElementById('alertbox_content').innerHTML = say + '<br style="clear:both" /><br />' +
		...
		engine: function () {
			...
			var generateEvents = function () {
				...
				if (prob > player) {
					...
					Game.say.alert("<h1>" + evt.msg + "<br>" + hpMsg + "<br>"+ fpMsg + "<br>"+ cashMsg + "</h1>");
					...
					if (gameVars.player.health < 30) {
						Game.say.alert("<h1>身体太差了，晕倒在大马路中央，差点被撞死。幸好热心的路人把俺送到医院，住了2天医院，花掉身上所有的钱。</h1>");
